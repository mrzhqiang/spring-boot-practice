...
(function (global, factory) {
  ...
  var preservedScriptAttributes = {
    type: true, src: true, nonce: true, noModule: true
  };
 ...
 */
      (function (window) {
        ...
        var i, support, Expr, getText, isXML, tokenize, compile, select, outermostContext,
            sortInput, hasDuplicate,

            // Local document vars
            setDocument, document, docElem, documentIsHTML, rbuggyQSA, rbuggyMatches, matches,
            contains,

            // Instance-specific data
            expando = "sizzle" + 1 * new Date(), preferredDoc = window.document, dirruns = 0,
            done = 0, classCache = createCache(), tokenCache = createCache(),
            compilerCache = createCache(), nonnativeSelectorCache = createCache(),
            ...
            // Instance methods
            hasOwn = ({}).hasOwnProperty, arr = [], pop = arr.pop, push_native = arr.push,
            push = arr.push,
            ...
            runescape = new RegExp("\\\\([\\da-f]{1,6}"
              ...
              return high !== high || escapedWhitespace ? escaped : high < 0 ? // BMP codepoint
                  String.fromCharCode(high + 0x10000) : // Supplemental Plane codepoint (surrogate
                                                        // pair)
        ...
        function Sizzle(selector, context, results, seed) {
          ...
          if (!seed) {
            ...
            if (documentIsHTML) {
              ...
              if (support.qsa && !nonnativeSelectorCache[selector + " "] && (!rbuggyQSA
         ...
         *     with property name the (space-suffixed) string and (if the cache is larger than
        ...
        function createDisabledPseudo(disabled) {
          ...
          return function (elem) {
            ...
            if ("form" in elem) {
              ...
              // Try to winnow out elements that can't be disabled before trusting the disabled
              // property. Some victims get caught in our net (label, legend, menu, track), but it
              // shouldn't even exist on them, let alone have a boolean value.
	...
	---------------------------------------------------------------------- */
          ...
          Expr.find["TAG"] = support.getElementsByTagName ? function (tag, context) {
              ...
              function (tag, context) {
                var elem, tmp = [],
                    i = 0, // By happy coincidence, a (broken) gEBTN appears on DocumentFragment
                           // nodes too
	...
	---------------------------------------------------------------------- */
          ...
          sortOrder = hasCompare ? function (a, b) {
            ...
            compare = (a.ownerDocument || a) === (b.ownerDocument
            ...
            if (!aup || !bup) {
              return a === document ? -1 : b
              === document ? 1 : aup ? -1 : bup ? 1 : sortInput ? (indexOf(sortInput, a)
        ...
        Sizzle.attr = function (elem, name) {
          ...
          var fn = Expr.attrHandle[name.toLowerCase()], // Don't get fooled by Object.prototype properties
                                              // (jQuery #13807)
              val = fn
              && hasOwn.call(Expr.attrHandle, name.toLowerCase()) ? fn(elem, name, !documentIsHTML) : undefined;

          return val !== undefined ? val : support.attributes
          || !documentIsHTML ? elem.getAttribute(name) : (val = elem.getAttributeNode(name))
        ...
        Expr = Sizzle.selectors = {
            ...
            "ATTR": function (name, operator, check) {
              return function (elem) {
                ...
                return operator === "=" ? result === check : operator === "!=" ? result
                    !== check : operator === "^=" ? check && result.indexOf(check) === 0 : operator
                === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check
                    && result.slice(-check.length)
                    === check : operator === "~=" ? (" "
                    + result.replace(rwhitespace, " ")
                    + " ").indexOf(check) > -1 : operator === "|=" ? result
                    === check
          ...
          pseudos: {
            ...
            // Boolean properties
            "enabled": createDisabledPseudo(false), "disabled": createDisabledPseudo(true),
            ...
            "empty": function (elem) {
              ...
              // 5), but not by others (comment: 8; processing instruction: 7; etc.) nodeType < 6
              // works because attributes (2) do not appear as children
              for (elem = elem.firstChild; elem; elem = elem.nextSibling) {
                if (elem.nodeType < 6) {
                  return false;
                }
          ...
          return markFunction(function (seed, results, context, xml) {
            var temp, i, elem, preMap = [], postMap = [], preexisting = results.length,
                ...
                matcherIn = preFilter && (seed
                    || !selector) ? condense(elems, preMap, preFilter, context, xml) : elems,

                matcherOut = matcher ? // If we have a postFinder, or filtered seed, or non-seed postFilter or preexisting
          // results,
            ...
            if (seed) {
              ...
              === results ? matcherOut.splice(preexisting, matcherOut.length) : matcherOut);
              if (postFinder) {
          ...
          for (; i < len; i++) {
            if ((matcher = Expr.relative[tokens[i].type])) {
              ...
              if (matcher[expando]) {
                ...
                return setMatcher(i > 1 && elementMatcher(matchers), i
                    > 1
                    && toSelector(// If the preceding token was a descendant combinator, insert an implicit any-element `*`
                        tokens.slice(0, i - 1).concat({
                          value: tokens[i - 2].type
                          === " " ? "*" : ""
                        })).replace(rtrim, "$1"), matcher, i
                    < j
                    && matcherFromTokens(tokens.slice(i, j)), j
                    < len
                    && matcherFromTokens((tokens = tokens.slice(j))), j
                    < len
                    && toSelector(tokens));
              }
          ...
          var bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0,
              superMatcher = function (seed, context, xml, results, outermost) {
                var elem, j, matcher, matchedCount = 0, i = "0", unmatched = seed && [],
                    setMatched = [],
                    contextBackup = outermostContext, // We must always have either seed elements
                                                      // or outermost context
                    elems = seed
                        || byElement
                        && Expr.find["TAG"]("*", outermost), // Use integer dirruns iff this is the
                                                             // outermost matcher
                ...
                // Support: IE<9, Safari
                // Tolerate NodeList properties (IE: "length"; Safari: <number>) matching elements
                // by id
                ...
                // `matchedCount` equals `i`), unless we didn't visit _any_ elements in the above
                // loop because we have no element matchers and no seed. Incrementing an
                // initially-string "0" `i` allows `i` to remain a string only in that case, which
                // will result in a "00" `matchedCount` that differs from `i` but is also
                // numerically zero.
 ...
 */
  ...
  function getDefaultDisplay(elem) {
    var temp, doc = elem.ownerDocument, nodeName = elem.nodeName,
  ...
  function buildFragment(elems, context, scripts, selection, ignored) {
    var elem, tmp, tag, wrap, attached, j, fragment = context.createDocumentFragment(), nodes = [],
        i = 0, l = elems.length;
 ...
 */
  jQuery.event = {
    ...
    add: function (elem, types, handler, data, selector) {
      ...
      var handleObjIn, eventHandle, tmp, events, t, handleObj, special, handlers, type, namespaces,
          origType, elemData = dataPriv.get(elem);
      ...
      if (!(eventHandle = elemData.handle)) {
        eventHandle = elemData.handle = function (e) {
          ...
          !== e.type ? jQuery.event.dispatch.apply(elem, arguments) : undefined;
        };
      }
    ...
    remove: function (elem, types, handler, selector, mappedTypes) {
      ...
      while (t--) {
        ...
        while (j--) {
          ...
          if ((mappedTypes || origType === handleObj.origType) && (!handler
              || handler.guid
            ...
            for (i = 0; i < delegateCount; i++) {
              ...
              if (matchedSelectors[sel] === undefined) {
                matchedSelectors[sel] = handleObj.needsContext ? jQuery(sel, this).index(cur)
                    > -1 : jQuery.find(sel, this, null, [cur]).length;
              }
  ...
  function leverageNative(el, type, expectSync) {
    ...
    jQuery.event.add(el, type, {
      namespace: false, handler: function (event) {
  ...
  function manipulationTarget(elem, content) {
    if (nodeName(elem, "table") && nodeName(content.nodeType
    !== 11 ? content : content.firstChild, "tr")) {
  ...
  function domManip(collection, args, callback, ignored) {
    ...
    var fragment, first, scripts, hasScripts, node, doc, i = 0, l = collection.length,
        iNoClone = l - 1, value = args[0], valueIsFunction = isFunction(value);

    // We can't cloneNode fragments that contain checked, in WebKit
    if (valueIsFunction || (l
        > 1
        && typeof value
        === "string"
        && !support.checkClone
  ...
  (function () {
    ...
    var pixelPositionVal, boxSizingReliableVal, scrollboxSizeVal, pixelBoxStylesVal,
        reliableMarginLeftVal, container = document.createElement("div"),
  ...
  var cssPrefixes = ["Webkit", "Moz", "ms"], emptyStyle = document.createElement("div").style,
      vendorProps = {};
  ...
  var
      ...
      rdisplayswap = /^(none|table(?!-c[ea]).+)/, rcustomProp = /^--/,
  ...
  function boxModelAdjustment(elem, dimension, box, isBorderBox, styles, computedVal) {
    ...
    if (!isBorderBox && computedVal >= 0) {
      ...
      delta += Math.max(0, Math.ceil(elem["offset"
      + dimension[0].toUpperCase()
      + dimension.slice(1)] - computedVal - delta - extra - 0.5

          // If offsetWidth/offsetHeight is unknown, then we can't determine content-box scroll
          // gutter Use an explicit zero to avoid NaN (gh-3964)
      )) || 0;
    }
  ...
  function getWidthOrHeight(elem, dimension, extra) {
    ...
    var styles = getStyles(elem),
        ...
        === "border-box", valueIsBorderBox = isBorderBox,
    ...
    if ((!support.boxSizingReliable()
        ...
        === "auto"
        || !parseFloat(val)
        && jQuery.css(elem, "display", false, styles)
        === "inline") && elem.getClientRects().length) {
    ...
    return (val + boxModelAdjustment(elem, dimension, extra
  ...
  jQuery.extend({
    ...
    style: function (elem, name, value, extra) {
      ...
      var ret, type, hooks, origName = camelCase(name), isCustomProp = rcustomProp.test(name),
          style = elem.style;
      ...
      get: function (elem, computed, extra) {
        if (computed) {
          ...
          (!elem.getClientRects().length
              || !elem.getBoundingClientRect().width) ? swap(elem, cssShow, function () {
  ...
  Tween.prototype = {
    constructor: Tween, init: function (elem, options, prop, end, easing, unit) {
      ...
      if (this.options.duration) {
        this.pos = eased = jQuery.easing[this.easing](percent, this.options.duration
      ...
      get: function (tween) {
        ...
        if (jQuery.fx.step[tween.prop]) {
          jQuery.fx.step[tween.prop](tween);
            ...
            || tween.elem.style[finalPropName(tween.prop)]
            != null)) {
  ...
  function createTween(value, prop, animation) {
    var tween, collection = (Animation.tweeners[prop] || []).concat(Animation.tweeners["*"]),
        index = 0, length = collection.length;
    for (; index < length; index++) {
    ...
    var result, stopped, index = 0, length = Animation.prefilters.length,
        deferred = jQuery.Deferred().always(function () {
          ...
          createTween: function (prop, end) {
            var tween = jQuery.Tween(elem, animation.opts, prop, end, animation.opts.specialEasing[prop]
  ...
  jQuery.each(["toggle", "show", "hide"], function (i, name) {
    ...
    jQuery.fn[name] = function (speed, easing, callback) {
      ...
      == null
      || typeof speed
      === "boolean" ? cssFn.apply(this, arguments) : this.animate(genFx(name, true), speed, easing, callback);
    };
  });
      ...
      return this.each(function () {
        ...
        if (isValidValue) {
          ...
          if (this.setAttribute) {
            ...
            || value
            === false ? "" : dataPriv.get(this, "__className__") || "");
          }
        }
      });
    },
  ...
  jQuery.extend(jQuery.event, {
    ...
    trigger: function (event, data, elem, onlyHandlers) {
      ...
      var i, cur, tmp, bubbleType, ontype, handle, special, lastElement,
          eventPath = [elem || document], type = hasOwn.call(event, "type") ? event.type : event,
      ...
      event = event[jQuery.expando] ? event : new jQuery.Event(type, typeof event
          === "object"
          && event);
  ...
  // http://www.w3.org/TR/DOM-Level-3-Events/#events-focusevent-event-order Related ticket -
  ...
  var r20 = /%20/g, rhash = /#.*$/, rantiCache = /([?&])_=[^&]*/,
      rheaders = /^(.*?):[ \t]*([^\r\n]*)$/mg,
 ...
 */
  ...
  jQuery.extend({
    ...
    ajax: function (url, options) {
      ...
      } else if (s.data && s.processData && (s.contentType
              ...
              if (callback) {
                ...
                if (type === "abort") {
                  ...
                  complete(xhrSuccessStatus[xhr.status] || xhr.status, xhr.statusText,
                      ...
                      !== "string" ? {binary: xhr.response} : {text: xhr.responseText}, xhr.getAllResponseHeaders());
                }
              }
            };
          };
  ...
  jQuery.ajaxPrefilter("json jsonp", function (s, originalSettings, jqXHR) {
    ...
    var callbackName, overwritten, responseContainer,
        jsonProp = s.jsonp !== false && (rjsonp.test(s.url) ? "url" : typeof s.data
                === "string"
                && (s.contentType || "")
  ...
  jQuery.fn.extend({
    ...
    offset: function (options) {
      ...
      return {
        top: rect.top + win.pageYOffset, left: rect.left + win.pageXOffset
      };
    },
      ...
      jQuery.fn[funcName] = function (margin, value) {
        ...
        return access(this, function (elem, type, value) {
          ...
          if (elem.nodeType === 9) {
            ...
            return Math.max(elem.body["scroll" + name], doc["scroll" + name], elem.body["offset"
